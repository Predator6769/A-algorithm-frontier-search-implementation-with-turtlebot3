...
class Task3(Node):
    ...
    def __init__(self):
        ...
        self.timer_shoot = self.create_timer(timer_period,self.shoot_through)
        self.timer_realign = self.create_timer(timer_period,self.realign_bot)
        ...
        self.focal_length = 1664
        self.object_width = 1.1
        ...
        self.encountered_obs = False
        self.shot_through = False
        self.linear_start_time = None
    ...
    def camera_callback(self, msg):
        ...
        lower_blue = np.array([120, 150, 50])
        upper_blue = np.array([140, 255, 255])

        # For dark green (and black under poor lighting)
        lower_bound = np.array([35, 50, 20])  # Lower Hue, Saturation, and Value for dark green
        upper_bound = np.array([85, 255, 100])  # Upper Hue, Saturation, and Value

        # For darker tones or black (fallback detection)
        lower_black = np.array([0, 0, 0])  # Lower bound for black
        upper_black = np.array([180, 50, 30])   # Black with a hint of other colors
        ...
        red_mask = cv2.morphologyEx(red_mask, cv2.MORPH_OPEN, kernel)  # Erosion followed by dilation (remove small noise)
        red_mask = cv2.morphologyEx(red_mask, cv2.MORPH_CLOSE, kernel)
        # plt.imshow(frame)
        # plt.show()
          # Dilation followed by erosion (close gaps)
...
# Find contours in the mask
        ...
        if len(contours) != 0:
            for contour in contours:
                if cv2.contourArea(contour) > min_contour_area:
                    ...
                    self.area = self.object_width*self.focal_length/w
                    print(f"Distance to object: {self.area}")
    ...
    def shoot_through(self):
        if self.encountered_obs and self.area > 5.5 and self.angular == True:
            if self.linear_start_time is None:
                self.linear_start_time = self.get_clock().now()
            self.twist.linear.x = 0.8
            elapsed_time = (self.get_clock().now() - self.linear_start_time).nanoseconds * 1e-9
            target_duration = 3.0 / abs(self.twist.linear.x)
            if elapsed_time >= target_duration:
                self.twist.linear.x = 0.0
                self.encountered_obs = False
                self.linear_start_time = None
                self.shot_through = True
                self.realigning = True

    def realign_bot(self):
        if self.realigning:
            min_dist = np.inf
            for i in range(0,len(self.path.poses)):
                linear_error = math.sqrt((self.odom.pose.pose.position.x  - self.path.poses[i].pose.position.x)**2 +(self.odom.pose.pose.position.y - self.path.poses[i].pose.position.y)**2)
                if min_dist > linear_error:
                    min_dist = linear_error
                    self.point_index = i
            self.realigning = False
            self.use_amcl = True
    ...
    def path_follow_callback(self):
        if (self.area > 4.0 and self.encountered_obs == False) or self.angular == False:
            self.angular = False
            if self.realigning == False:
                if len(self.path.poses)!=0:
                    if self.point_index < len(self.path.poses) - 1:
                        if self.use_amcl == False:
                            l = self.path.poses[self.point_index]
                        else:
                            l = self.odom.pose
                        ...
                        orientation_target = (math.atan2(self.path.poses[self.point_index+1].pose.position.y - l.pose.position.y ,self.path.poses[self.point_index+1].pose.position.x - l.pose.position.x ))
            ...
            if self.encountered_obs == False and self.shot_through == False:
               self.encountered_obs = True
               self.twist.linear.x = 0.0
               self.twist.angular.z = 0.0
